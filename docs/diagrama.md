...
- [ ] **Testes instrumentados de feature** (`src/androidTest/`) — validação final na tela real, conforme a política de dois níveis definida no Passo 23. Prontos: `GameCard` (template), **Catálogo** e **Lista Pessoal** (Passo 33). Faltam **Detalhes** e **Busca**. Cada um é lento (~70s, roda no aparelho físico SM-S908E) — o Claude **pede permissão a Igor antes de executar** cada um (`connectedDebugAndroidTest`), como combinado no Passo 23.
...
*Próximo passo: a definir com Igor — seguir com os testes instrumentados restantes (Detalhes e Busca, pedindo permissão antes de rodar cada um), esclarecer o spike de widget, ou seguir para a Fase 3.*
